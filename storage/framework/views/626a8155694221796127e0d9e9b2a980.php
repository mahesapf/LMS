<?php $__env->startSection('title', 'Pemetaan Admin'); ?>
...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="<?php echo e(route('super-admin.admin-mappings.create')); ?>" class="inline-flex items-center gap-2 rounded-lg bg-sky-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-sky-700">
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-2 text-sm">
                            <div class="text-slate-900 font-semibold"><?php echo e($mapping->admin->name); ?></div>
                            <div class="text-xs text-slate-500"><?php echo e($mapping->admin->email); ?></div>
                        </td>
                        <td class="px-4 py-2 text-sm text-slate-700"><?php echo e($mapping->program ? $mapping->program->name : '-'); ?></td>
                        <td class="px-4 py-2 text-sm text-slate-700"><?php echo e($mapping->activity ? $mapping->activity->name : '-'); ?></td>
                        <td class="px-4 py-2 text-sm">
                            <?php if($mapping->status == 'in'): ?>
                        ...
                        <td class="px-4 py-2 text-sm text-slate-700"><?php echo e($mapping->assigned_date ? $mapping->assigned_date->format('d/m/Y') : '-'); ?></td>
                        <td class="px-4 py-2 text-sm text-slate-700"><?php echo e($mapping->removed_date ? $mapping->removed_date->format('d/m/Y') : '-'); ?></td>
                        <td class="px-4 py-2 text-sm text-slate-700"><?php echo e($mapping->assignedBy->name); ?></td>
                        <td class="px-4 py-2 text-sm">
                            <div class="flex justify-end gap-2">
                                <?php if($mapping->status == 'in'): ?>
                                    <form method="POST" action="<?php echo e(route('super-admin.admin-mappings.update-status', [$mapping, 'out'])); ?>" onsubmit="return confirm('Keluarkan admin dari tugas ini?')">
                                        <?php echo csrf_field(); ?>
                                ...
                                <?php else: ?>
                                    <form method="POST" action="<?php echo e(route('super-admin.admin-mappings.update-status', [$mapping, 'in'])); ?>">
                                        <?php echo csrf_field(); ?>
        ...
        <div class="mt-4"><?php echo e($mappings->links()); ?></div>
    </div>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\LMS\resources\views/super-admin/admin-mappings/index.blade.php ENDPATH**/ ?>