...
<div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" @keydown.escape.window="showEditModal = false">
    ...
    <div class="relative z-10 flex min-h-screen items-center justify-center p-4">
        <div @click.away="showEditModal = false" class="relative w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200 max-h-[90vh] overflow-y-auto"
             x-data="{
                 stageCount: <?php echo e($editClass->stages->count()); ?>,
                 stages: <?php echo e(json_encode($editClass->stages->map(function($s) { return ['name' => $s->name, 'description' => $s->description, 'order' => $s->order, 'start_date' => $s->start_date ? $s->start_date->format('Y-m-d') : '', 'end_date' => $s->end_date ? $s->end_date->format('Y-m-d') : '', 'status' => $s->status]; })->values())); ?>

             }">
        <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4">
            <div>
                <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Formulir</p>
                <h2 class="text-lg font-semibold text-slate-900">Edit Kelas</h2>
            </div>
        ...
        <form action="<?php echo e(route($routePrefix . '.classes.update', $editClass)); ?>" method="POST" class="divide-y divide-slate-100">
            <?php echo csrf_field(); ?>
            ...
            <div class="space-y-6 px-6 py-6">
                <div>
                    ...
                    <select name="activity_id" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm <?php $__errorArgs = ['activity_id'];
...
unset($__errorArgs, $__bag); ?>" required>
                        ...
                        <option value="<?php echo e($activity->id); ?>" <?php echo e(old('activity_id', $editClass->activity_id) == $activity->id ? 'selected' : ''); ?>><?php echo e($activity->name); ?></option>
...
unset($__errorArgs, $__bag); ?>" value="<?php echo e(old('name', $editClass->name)); ?>" required>
...
unset($__errorArgs, $__bag); ?>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm <?php $__errorArgs = ['description'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>"><?php echo e(old('description', $editClass->description)); ?></textarea>
...
unset($__errorArgs, $__bag); ?>" value="<?php echo e(old('start_date', $editClass->start_date ? $editClass->start_date->format('Y-m-d') : '')); ?>">
...
unset($__errorArgs, $__bag); ?>" value="<?php echo e(old('end_date', $editClass->end_date ? $editClass->end_date->format('Y-m-d') : '')); ?>">
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="max_participants" min="1" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm <?php $__errorArgs = ['max_participants'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>" value="<?php echo e(old('max_participants', $editClass->max_participants)); ?>">
...
unset($__errorArgs, $__bag); ?>" required>
                            <option value="open" <?php echo e(old('status', $editClass->status) == 'open' ? 'selected' : ''); ?>>Buka</option>
                            <option value="closed" <?php echo e(old('status', $editClass->status) == 'closed' ? 'selected' : ''); ?>>Tutup</option>
                            <option value="completed" <?php echo e(old('status', $editClass->status) == 'completed' ? 'selected' : ''); ?>>Selesai</option>
                        </select>
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\LMS\resources\views/super-admin/classes/partials/edit-modal.blade.php ENDPATH**/ ?>