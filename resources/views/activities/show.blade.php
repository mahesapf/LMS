    ...
    <div class="max-w-6xl mx-auto px-4 py-10">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
            ...
            <div class="flex items-center gap-2 text-xs text-slate-500">
                @if($activity->program)
                    <span class="inline-flex items-center gap-1 rounded-full bg-blue-50 px-3 py-1 font-semibold text-blue-700 ring-1 ring-blue-100">{{ $activity->program->name }}</span>
                @endif
                @if($activity->batch)
                    <span class="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-3 py-1 font-semibold text-indigo-700 ring-1 ring-indigo-100">Batch {{ $activity->batch }}</span>
                @endif
            </div>
        </div>
                    ...
                    <div class="p-6">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                            <div class="space-y-2">
                                ...
                                <h1 class="text-2xl font-semibold text-slate-900">{{ $activity->name }}</h1>
                                <div class="flex flex-wrap gap-2 text-sm text-slate-600">
                                    <span class="inline-flex items-center gap-2 rounded-full {{ $badgeClass }} px-3 py-1 font-semibold ring-1">
                                    ...
                                    <span class="inline-flex items-center gap-2 rounded-full bg-amber-50 px-3 py-1 font-semibold text-amber-800 ring-1 ring-amber-100">
                                        Mulai {{ $activity->start_date->format('d F Y') }}
                                    </span>
                                </div>
                            </div>
                            <div class="rounded-xl bg-gradient-to-br from-sky-50 to-white px-4 py-3 text-sm text-sky-900 ring-1 ring-sky-100 shadow-inner">
                                ...
                                @if($activity->registration_fee > 0)
                                    <p class="text-xl font-bold mt-1">Rp {{ number_format($activity->registration_fee, 0, ',', '.') }} <span class="text-sm font-semibold text-slate-500">/peserta</span></p>
                                @else
                        ...
                        @if($activity->description)
                            <div class="mt-6 border-t border-slate-100 pt-6">
                                ...
                                <p class="mt-2 text-slate-700 leading-relaxed">{{ $activity->description }}</p>
                            </div>
                        @endif
                            ...
                            <div class="rounded-xl border border-slate-100 bg-slate-50 p-4">
                                ...
                                <dl class="mt-3 space-y-2 text-sm text-slate-700">
                                    <div class="flex justify-between">
                                        ...
                                        <dd class="font-semibold">{{ $activity->start_date->format('d F Y') }}</dd>
                                    </div>
                                    <div class="flex justify-between">
                                        <dt class="text-slate-500">Tanggal Selesai</dt>
                                        <dd class="font-semibold">{{ $activity->end_date->format('d F Y') }}</dd>
                                    </div>
                                    @if($activity->batch)
                                        <div class="flex justify-between">
                                            <dt class="text-slate-500">Batch</dt>
                                            <dd class="font-semibold">{{ $activity->batch }}</dd>
                                        </div>
                                    @endif
                                </dl>
                            </div>
                                ...
                                <dl class="mt-3 space-y-2 text-sm text-slate-700">
                                    @if($activity->financing_type)
                                        <div class="flex justify-between">
                                            ...
                                            <dd class="font-semibold">{{ $activity->financing_type }}</dd>
                                        </div>
                                    @endif
                                    @if($activity->apbn_type)
                                        <div class="flex justify-between">
                                            <dt class="text-slate-500">Tipe APBN</dt>
                                            <dd class="font-semibold">{{ $activity->apbn_type }}</dd>
                                        </div>
                                    @endif
                                    <div class="flex justify-between">
                                        ...
                                        <dd class="font-semibold">
                                            @if($activity->registration_fee > 0)
                                                Rp {{ number_format($activity->registration_fee, 0, ',', '.') }} / peserta
                                            @else
                                                Gratis
                                            @endif
                                        </dd>
                        ...
                        @if($existingRegistration)
                            ...
                            <div class="mt-2 space-y-3">
                                @if($existingRegistration->status == 'payment_pending')
                                    ...
                                    <a href="{{ route('payment.create', $existingRegistration) }}" class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-sky-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-500">
                                        Upload Pembayaran
                                    </a>
                    ...
                    <div class="bg-white px-6 pt-5 pb-4">
                        <div class="flex items-start justify-between border-b border-slate-200 pb-3">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-slate-600">{{ $activity->name }}</p>
                            </div>
                        ...
                        <div class="mt-5">
                            ...
                            <div class="mb-4 grid gap-4 sm:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <select class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-sky-500 focus:ring-sky-500 @error('provinsi') border-red-500 @enderror"
                                           id="provinsi_show"
                                           name="provinsi"
                                           required
                                ...
                                <div>
                                    ...
                                    <select class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-sky-500 focus:ring-sky-500 @error('kab_kota') border-red-500 @enderror"
                                           id="kab_kota_show"
                                           name="kab_kota"
                                           required
                                ...
                                <div>
                                    ...
                                    <select class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-sky-500 focus:ring-sky-500 @error('kecamatan') border-red-500 @enderror"
                                           id="kecamatan_show"
                                           name="kecamatan"
                                           required>
                                ...
                                <div class="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-4">
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="text-sm text-amber-800">
                                            ...
                                            <p class="mt-1">Rp {{ number_format($activity->registration_fee, 0, ',', '.') }} per peserta. Total biaya dihitung dari jumlah peserta yang didaftarkan.</p>
                                        </div>
                                    </div>
                                </div>
                            @endif
