...
<div class="container-fluid">
    <div class="mb-4">
        <h1>Edit Kegiatan</h1>
        <a href="{{ route($routePrefix . '.activities') }}" class="btn btn-secondary">Kembali</a>
    </div>

    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route($routePrefix . '.activities.update', $activity) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="4">{{ old('description', $activity->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="batch" class="form-control @error('batch') is-invalid @enderror" value="{{ old('batch', $activity->batch) }}" placeholder="Contoh: Batch 1, Angkatan 2024">
                    @error('batch')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        ...
                        <select name="financing_type" id="financing_type" class="form-select @error('financing_type') is-invalid @enderror">
                            ...
                            <option value="APBN" {{ old('financing_type', $activity->financing_type) == 'APBN' ? 'selected' : '' }}>APBN</option>
                            <option value="Non-APBN" {{ old('financing_type', $activity->financing_type) == 'Non-APBN' ? 'selected' : '' }}>Non-APBN</option>
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="apbn_type" id="apbn_type" class="form-select @error('apbn_type') is-invalid @enderror">
                            ...
                            <option value="BOS Reguler" {{ old('apbn_type', $activity->apbn_type) == 'BOS Reguler' ? 'selected' : '' }}>BOS Reguler</option>
                            <option value="BOS Kinerja" {{ old('apbn_type', $activity->apbn_type) == 'BOS Kinerja' ? 'selected' : '' }}>BOS Kinerja</option>
                            <option value="DIPA" {{ old('apbn_type', $activity->apbn_type) == 'DIPA' ? 'selected' : '' }}>DIPA</option>
                        </select>
                        @error('apbn_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="registration_fee" class="form-control @error('registration_fee') is-invalid @enderror"
                               value="{{ old('registration_fee', $activity->registration_fee ?? 0) }}" min="0" step="0.01">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route($routePrefix . '.activities') }}" class="btn btn-secondary">Batal</a>
                </div>
            </form>
        </div>
    </div>
