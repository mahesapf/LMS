...
<div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" @keydown.escape.window="showEditModal = false">
    ...
    <div class="relative z-10 flex min-h-screen items-center justify-center p-4">
        <div @click.away="showEditModal = false" class="relative w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200 max-h-[90vh] overflow-y-auto"
             x-data="{
                 stageCount: {{ $editClass->stages->count() }},
                 stages: {{ json_encode($editClass->stages->map(function($s) { return ['name' => $s->name, 'description' => $s->description, 'order' => $s->order, 'start_date' => $s->start_date ? $s->start_date->format('Y-m-d') : '', 'end_date' => $s->end_date ? $s->end_date->format('Y-m-d') : '', 'status' => $s->status]; })->values()) }}
             }">
        ...
        <form action="{{ route($routePrefix . '.classes.update', $editClass) }}" method="POST" class="divide-y divide-slate-100">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6 px-6 py-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm @error('name') border-red-500 @enderror" value="{{ old('name', $editClass->name) }}" required>
                    @error('name')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm @error('description') border-red-500 @enderror">{{ old('description', $editClass->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="date" name="start_date" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm @error('start_date') border-red-500 @enderror" value="{{ old('start_date', $editClass->start_date ? $editClass->start_date->format('Y-m-d') : '') }}">
                        @error('start_date')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="date" name="end_date" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm @error('end_date') border-red-500 @enderror" value="{{ old('end_date', $editClass->end_date ? $editClass->end_date->format('Y-m-d') : '') }}">
                        @error('end_date')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="max_participants" min="1" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm @error('max_participants') border-red-500 @enderror" value="{{ old('max_participants', $editClass->max_participants) }}">
                    ...
                    <div>
                        ...
                        <select name="status" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm @error('status') border-red-500 @enderror" required>
                            <option value="open" {{ old('status', $editClass->status) == 'open' ? 'selected' : '' }}>Buka</option>
                            <option value="closed" {{ old('status', $editClass->status) == 'closed' ? 'selected' : '' }}>Tutup</option>
                            <option value="completed" {{ old('status', $editClass->status) == 'completed' ? 'selected' : '' }}>Selesai</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
